...
read_btn <- function(file = {cat('Please select btn file ...\n'); file.choose()}) {
  ...
  # Data set A4
    data_set_a4 <- RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]])
    btn_lines <- btn_lines[-1]  
  ...
  # Data set A5
    btn$trnop <- as.logical(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
    btn_lines <- btn_lines[-1]  
    
  # Data set A6
    btn$laycon <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
    btn_lines <- btn_lines[-1]
  ...
  # Data set A14
    data_set_a14 <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
  ...
  # Data set A15
    data_set_a15 <- RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]])
  ...
  # Data set A17
    if(btn$nprs > 0) {
      ...
      btn$timprs <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(paste(btn_lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]]))
      btn_lines <- btn_lines[-c(1:nLines)]
    }
  
  # Data set A18
    data_set_a18 <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
  ...
  # Data set A19
    if(btn$nobs > 0) {
      ...
      btn$jobs <- NULL
      for(i in 1:btn$nobs) {
        data_set_a19 <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
  ...
  # Data set A20
    data_set_a20 <- RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]])
  ...
  for(i in 1:btn$nper) {  
    # Data set A21
      data_set_a21 <- RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]])
    ...
    # Data set A22
      if(btn$tsmult[i] <= 0) {
        ...
        btn$tslngh[[i]] <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit0(paste(btn_lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]]))
        btn_lines <- btn_lines[-c(1:nLines)]
      }
    
    # Data set A23
      data_set_a23 <- as.numeric(RMODFLOW:::rmfi_remove_empty_strings(strsplit(btn_lines[1],' ')[[1]]))
